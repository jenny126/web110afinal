...
db.query("CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, title TEXT, body TEXT,date TEXT)");
db.query("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, password TEXT, email TEXT,date TEXT)");
...
function postQuery(sql) {
  let list = []
  for (const [date,id, username, title, body] of sqlcmd(sql)) {
    list.push({date,id, username, title, body})
  }
...
function userQuery(sql) {
  let list = []
  for (const [date,id, username, password, email] of sqlcmd(sql)) {
    list.push({date,id, username, password, email})
  }
...
async function list(ctx) {
  let posts = postQuery("SELECT id, username, date,title, body FROM posts")
...
async function create(ctx) {
  ...
  if (body.type === "form") {
    ...
    if (user != null) {
      ...
      sqlcmd("INSERT INTO posts (username, title, body,date) VALUES (?, ?, ?,?)", [user.username, post.title, post.body,post.date]);  
    } else {
...
console.log('Server run at http://127.0.0.1:8016')
